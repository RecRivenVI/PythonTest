...
# 读取 CSV 文件
df = pd.read_csv('timeline_data.csv', encoding='utf-8-sig')
df['拍摄时间'] = pd.to_datetime(df['拍摄时间'])
    ...
    if merge_option == 'merge':
        ...
        for user, user_df in filtered_df.groupby('用户'):
            fig.add_trace(go.Scattergl(
                ...
                text=[f"{row['用户']} - {row['设备型号']}<br>文件类型: {row['文件类型']}" for _, row in user_df.iterrows()],
                hovertemplate=( 
                    '<b>%{text}</b><br><br>' +
                    '拍摄时间: %{x|%Y-%m-%d %H:%M:%S}<br>' +
                    '<extra></extra>'
                )
            ))
    else:
        ...
        for user, user_df in filtered_df.groupby('用户'):
            for device, device_df in user_df.groupby('设备型号'):
                fig.add_trace(go.Scattergl(
                    ...
                    name=f"{user} - {device}",
                    marker=dict(size=10, color=colors[user]),  # 根据用户选择颜色
                    text=[f"{row['用户']} - {row['设备型号']}<br>文件类型: {row['文件类型']}" for _, row in device_df.iterrows()],
                    hovertemplate=( 
                        '<b>%{text}</b><br><br>' +
                        '拍摄时间: %{x|%Y-%m-%d %H:%M:%S}<br>' +
                        '设备: %{y}<br>' +
